  ...
  app.get("/api/games/:id/state", async (req, res) => {
    try {
      ...
      const hasAttemptAfterChoice =
        ...
        events.slice(lastConversionChoiceIdx + 1).some((e) => e.type === "pat_attempt" || e.type === "two_point_attempt");
      const awaitingConversionAttempt = lastEvent?.type === "conversion_choice" && !hasAttemptAfterChoice;

      const pendingConversionType = awaitingConversionAttempt ? ((lastEvent?.data as Record<string, unknown>)?.type as string) : null;
  ...
  app.post("/api/games/:id/ot-coin-flip", async (req, res) => {
    try {
      ...
      await storage.createEvent({
        ...
        description: choice === "receive" ? `OT coin flip won! Elects to receive.` : `OT coin flip won! Defers - opponent receives.`,
      });
