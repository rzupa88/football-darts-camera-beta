...
function getHitType(multiplier: Multiplier, segment: number): string {
...
export async function registerRoutes(
  ...
  app.get("/api/games/:id/state", async (req, res) => {
    try {
      ...
      const hasAttemptAfterChoice = lastConversionChoiceIdx >= 0 && events.slice(lastConversionChoiceIdx + 1).some(e => 
        e.type === "pat_attempt" || e.type === "two_point_attempt"
      );
      ...
      const awaitingBonusDart = !!(lastEvent?.data && 
      ...
      const availableActions = getAvailableActions(game, currentDrive, awaitingConversion, awaitingConversionAttempt, awaitingBonusDart);
  ...
  app.post("/api/games/:id/ot-coin-flip", async (req, res) => {
    try {
      ...
      const receiverPlayerId = firstPossession === 1 ? game.player1Id : game.player2Id;
  ...
  app.post("/api/games/:id/action", async (req, res) => {
    try {
      ...
      const { segment, multiplier, action } = req.body;
      const currentDrive = await storage.getCurrentDrive(game.id);
      ...
      if (isConversion) {
        ...
        if (conversionType === "pat") {
          const patTargets = [1, 5, 20]; // Top arc of dartboard
          success = multiplier !== "miss" && patTargets.includes(segment);
          points = success ? 1 : 0;
        } else {
          success = segment === 2 && multiplier !== "miss";
          points = success ? 2 : 0;
        }
        ...
        if (currentDrive) {
          ...
          const dartThrow: InsertDartThrow = {
            ...
            rulePath: success ? (conversionType === "pat" ? "PAT_GOOD" : "TWO_GOOD") : (conversionType === "pat" ? "PAT_MISS" : "TWO_MISS"),
          };
      ...
      if (action === "punt") {
        ...
        if (insideOwn30Penalty > 0) {
          puntDescription = `${puntResult.description} (Punting from own ${puntingPosition}: -${insideOwn30Penalty} yards penalty, opponent starts at ${formatFieldPosition(adjustedReceivingPosition)})`;
        }
      ...
      if (isInterception) {
        ...
        await storage.createEvent({
          ...
          description: `INTERCEPTION! ${formatDartResult(dartResult)} - Turnover at ${formatFieldPosition(currentDrive.currentPosition)}. Opponent takes over at ${formatFieldPosition(opponentStartPosition)}.`,
        });
...
function getAvailableActions(game: any, currentDrive: any, awaitingConversion: boolean = false, awaitingConversionAttempt: boolean = false, awaitingBonusDart: boolean = false) {
  ...
  if (game.status === "overtime" || game.currentQuarter >= 5) {
    ...
    if (bothCompletedPeriod) {
      ...
      if (updatedGame.player1Score !== updatedGame.player2Score) {
        const winnerId = updatedGame.player1Score > updatedGame.player2Score 
          ? game.player1Id 
          : game.player2Id;
...
function formatDartResult(dart: DartResult): string {
  ...
  const prefix = dart.multiplier === "triple" ? "T" 
